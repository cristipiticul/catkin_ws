...
// Name of frame in which you are expressing poses. Typically "world_frame" or "base_link".
  const std::string world_frame = "/base_footprint";

// tool center point frame (name of link associated with tool)
  const std::string tcp_frame = "wrist_roll";
...
// 3. Create a planner and initialize it with our robot model
  ...
  for (int i = 0; i < 10; i++) {
      Eigen::Affine3d pose;
      pose = Eigen::Translation3d(0.1, 0.1, -0.02 * i);
...
// a certain time delta between each trajectory point
  trajectory_msgs::JointTrajectory joint_solution = toROSJointTrajectory(result,
      *model, names, 1.0);
...
descartes_core::TrajectoryPtPtr makeTolerancedCartesianPoint(
  ...
  using namespace descartes_core;
  using namespace descartes_trajectory;
  return TrajectoryPtPtr(
      new AxialSymmetricPt(pose, M_PI / 2.0 - 0.0001, AxialSymmetricPt::Z_AXIS));
}
